...
def find_closest_centroids(samples, centroids):
    ...
    """
    cluster_id = np.zeros(len(samples))
    for i in range(len(samples)):
        store_list = []
        for j in range(len(centroids)):
            dis = distance.euclidean(samples[i],centroids[j])
            store_list.append(dis)
        cluster_id[i] = np.argmin(store_list)
    return cluster_id
...
def choose_random_centroids(samples, K):
    ...
    for i in range(K):
        centroids[i] = samples[np.random.randint(low = 0, high = len(samples))]
    return centroids
